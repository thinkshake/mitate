...
import { type Bet } from "@/lib/user-data"

export function ActiveBets({ bets }: { bets: Bet[] }) {
  if (bets.length === 0) {
    return (
      <section aria-label="アクティブな予測" className="mt-12">
            ...
            <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-sm">
              <div>
                ...
                <span className="font-mono text-foreground">
                  ¥{bet.amount.toLocaleString("ja-JP")} JPYC
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">重み: </span>
                <span className="font-mono text-foreground">
                  {"\u00D7"}{bet.weight.toFixed(1)}
                </span>
              </div>
              <div>
                <span className="text-muted-foreground">実効額: </span>
                <span className="font-mono text-foreground">
                  ¥{bet.effectiveAmount.toLocaleString("ja-JP")} JPYC
                </span>
              </div>
            </div>
